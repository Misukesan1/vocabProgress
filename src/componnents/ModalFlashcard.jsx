import { Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, Input, Form, Textarea } from "@heroui/react"
import { useEffect, useState } from "react"
import { addFlashcard, editFlashcard } from "../database/flashcard"
import { useDispatch } from "react-redux"
import { showAlert } from "../features/alertSlice"

export default function ModalFlashcard({ isOpen, onOpenChange, flashcard = null, ficheId }) {
    ...
    const handleSubmit = async (e, onClose) => {
        ...
        try {
            if (isNewFlashCard) {
                ...
                dispatch(showAlert({message: "Nouvelle flashcard créée.", type: "success"}))
                onClose()
            } else {
                await editFlashcard(flashcard.id, frontCard, backCard)
                dispatch(showAlert({message: "Flashcard modifiée.", type: "success"}))
                onClose()
            }
            ...
            <ModalContent>
                {(onClose) => (
                    <Form className="contents" onSubmit={(e) => handleSubmit(e, onClose)}>
                        <ModalHeader>{isNewFlashCard ? "Nouvelle flashcard." : "Modifier la flashcard."}</ModalHeader>
                        ...
                        <ModalFooter>
                            <Button variant="light" color="danger" onPress={onClose}>Retour</Button>
                            <Button color="primary" type="submit">{isNewFlashCard ? "Créer" : "Modifier"}</Button>
                        </ModalFooter>
                    </Form>
                )}
            </ModalContent>
